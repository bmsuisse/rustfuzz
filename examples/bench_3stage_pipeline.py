...
def evaluate_ndcg_at_10(
    qrels: dict[str, dict[str, int]], runs: dict[str, list[str]]
) -> float:
    ndcg_list = []
    import math
...
def main():
    ...
    bge_model = embed_anything.Reranker.from_pretrained(
        "llmware/jina-reranker-tiny-onnx"
    )
    ...
    for bench in benches:
        ...
        for qid in tqdm(test_qids, desc="Retrieving"):
            ...
            # Execute
            if bench["use_fuzzy"]:
                res = query_builder.match(
                    query_text,
                    method="get_top_n_fuzzy",
                    n=40 if bench["use_reranker"] else 10,
                )
            else:
                res = query_builder.match(
                    query_text, n=40 if bench["use_reranker"] else 10
                )
            ...
            # Map back to doc IDs
            runs[qid] = [
                doc_text_to_id[doc] for doc, score in res if doc in doc_text_to_id
            ]
        ...
        print(
            f"Result: {name} | Latency: {avg_latency:.2f}ms/query | nDCG@10: {ndcg:.4f}"
        )
    ...
    for i in range(len(names)):
        plt.scatter(
            latencies[i], ndcgs[i], s=150, c=colors[i], label=names[i], alpha=0.8
        )
