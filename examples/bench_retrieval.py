...
def benchmark_variant(
    variant_class, name, corpus_docs, doc_text_to_id, queries, qrels, kwargs=None
):
    if kwargs is None:
        kwargs = {}
    print(f"\\nEvaluating {name}...")
    ...
    for qid in tqdm(test_qids, desc="Retrieving"):
        ...
        runs[qid] = [
            doc_text_to_id[doc] for doc, score in results if doc in doc_text_to_id
        ]
    ...
    print(
        f"{name} - Indexing: {indexing_time:.2f}ms | Latency: {avg_latency:.2f}ms/query | nDCG@10: {ndcg:.4f}"
    )
...
def main():
    ...
    for variant_class, name, kwargs in variants:
        latency, ndcg = benchmark_variant(
            variant_class, name, corpus_docs, doc_text_to_id, queries, qrels, kwargs
        )
