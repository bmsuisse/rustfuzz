...
def bench_search_3way(hs: object, query: str, q_emb: list[float], label: str, n: int = 10, bm25_cands: int = 200) -> None:
    ...
    print(f"  [{label}] 3-way search (n={n}, cands={bm25_cands}): {avg_ms:>8.3f} ms/query")


def bench_search_2way(hs: object, query: str, label: str, n: int = 10, bm25_cands: int = 200) -> None:
    ...
    t1 = time.perf_counter()
    avg_ms = (t1 - t0) / runs * 1000
    print(f"  [{label}] 2-way search (n={n}, cands={bm25_cands}): {avg_ms:>8.3f} ms/query")
