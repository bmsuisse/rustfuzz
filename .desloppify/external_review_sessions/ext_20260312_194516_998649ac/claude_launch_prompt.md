...
Template JSON: /Users/dominikpeter/DevOps/RapidFuzz/.desloppify/external_review_sessions/ext_20260312_194516_998649ac/review_result.template.json
Output JSON path: /Users/dominikpeter/DevOps/RapidFuzz/.desloppify/external_review_sessions/ext_20260312_194516_998649ac/review_result.json
...
- rustfuzz/filter.py
- examples/bench_3stage_pipeline.py
- examples/bench_retrieval.py
...
- rustfuzz/__init__.py
- rustfuzz/compat.py
- rustfuzz/document.py
- examples/azure_retriever_example.py
- examples/bench_3stage_pipeline.py
- examples/bench_crush_ndcg.py
...
then read the actual source code.
  ...
  desloppify show boilerplate_duplication --no-budget      # 9 findings
  desloppify show dupes --no-budget      # 14 findings
  desloppify show signature --no-budget      # 4 findings
...
then read the actual source code.
  ...
  desloppify show security --no-budget      # 10 findings
  desloppify show smells --no-budget      # 52 findings
...
then read the actual source code.
  ...
  desloppify show structural --no-budget      # 7 findings
...
Seed files (start here):
- examples/embed_anything_examples.py
- examples/filter_sort_examples.py
- examples/quickstart.py

--- Batch 11: incomplete_migration ---
...
- examples/embed_anything_examples.py
- examples/azure_retriever_example.py
- examples/multijoiner_examples.py
- rustfuzz/langchain.py
- examples/blazing_fast_pipeline.py
- examples/filter_sort_examples.py
- rustfuzz/distance/Gotoh.py
- rustfuzz/join.py
- rustfuzz/compat.py
...
Overview (16 signals):
  design_concern: 8 — examples/azure_retriever_example.py, examples/bench_retrieval.py, ...
  mixed_responsibilities: 7 — examples/bench_3stage_pipeline.py, examples/bench_crush_ndcg.py, ...
  systemic_smell: 1 — rustfuzz/compat.py
...
For each concern, read the source code and report your verdict in issues[]:
  ...
  - [design_concern] examples/azure_retriever_example.py
    ...
    fingerprint: fc97c5053826b967
  - [design_concern] examples/bench_retrieval.py
    summary: Design signals from orphaned, smells
    question: Is this file truly dead, or is it used via a non-import mechanism (dynamic import, CLI entry point, plugin)?
    evidence: Flagged by: orphaned, smells
    evidence: [orphaned] Orphaned file (183 LOC): zero importers, not an entry point
    fingerprint: 191f4b62198ef289
  - [design_concern] examples/blazing_fast_pipeline.py
    summary: Design signals from orphaned, smells
    question: Is this file truly dead, or is it used via a non-import mechanism (dynamic import, CLI entry point, plugin)?
    evidence: Flagged by: orphaned, smells
    evidence: [orphaned] Orphaned file (117 LOC): zero importers, not an entry point
    fingerprint: c6f7cc38cf88e34f
  - [design_concern] examples/filter_sort_examples.py
    summary: Design signals from orphaned, smells
    question: Is this file truly dead, or is it used via a non-import mechanism (dynamic import, CLI entry point, plugin)?
    evidence: Flagged by: orphaned, smells
    evidence: [orphaned] Orphaned file (325 LOC): zero importers, not an entry point
    fingerprint: 6153d655c995f49e
  - [design_concern] examples/multijoiner_examples.py
    summary: Design signals from orphaned, smells
    question: Is this file truly dead, or is it used via a non-import mechanism (dynamic import, CLI entry point, plugin)?
    evidence: Flagged by: orphaned, smells
    evidence: [orphaned] Orphaned file (348 LOC): zero importers, not an entry point
    fingerprint: fd6087da9d5dc7e4
  ...
  - [mixed_responsibilities] examples/bench_3stage_pipeline.py
    summary: Issues from 5 detectors — may have too many responsibilities
    question: This file has issues across 5 dimensions (boilerplate_duplication, dupes, orphaned, signature, smells). Is it trying to do too many things, or is this complexity inherent to its domain? Is the duplication worth extracting into a shared utility, or is it intentional variation? Is this file truly dead, or is it used via a non-import mechanism (dynamic import, CLI entry point, plugin)?
    evidence: Flagged by: boilerplate_duplication, dupes, orphaned, signature, smells
    evidence: [orphaned] Orphaned file (180 LOC): zero importers, not an entry point
    fingerprint: ab09153019c5bdd7
  - [mixed_responsibilities] examples/bench_crush_ndcg.py
    summary: Issues from 5 detectors — may have too many responsibilities
    question: This file has issues across 5 dimensions (boilerplate_duplication, orphaned, responsibility_cohesion, smells, structural). Is it trying to do too many things, or is this complexity inherent to its domain? Should the parameters be grouped into a config/context object? Which ones belong together? Can the nesting be reduced with early returns, guard clauses, or extraction into helper functions? Is the duplication worth extracting into a shared utility, or is it intentional variation? Is this file truly dead, or is it used via a non-import mechanism (dynamic import, CLI entry point, plugin)? What are the distinct responsibilities? Would splitting produce modules with multiple independent consumers, or would extracted files only be imported by the parent? Only split if the extracted code would be reused.
    evidence: Flagged by: boilerplate_duplication, orphaned, responsibility_cohesion, smells, structural
    evidence: File size: 344 lines
    fingerprint: 22db364aad610b50
  - [mixed_responsibilities] examples/embed_anything_examples.py
    ...
    evidence: [orphaned] Orphaned file (307 LOC): zero importers, not an entry point
    fingerprint: 7b637d83f6519131
  - [mixed_responsibilities] examples/quickstart.py
    summary: Issues from 3 detectors — may have too many responsibilities
    question: This file has issues across 3 dimensions (orphaned, smells, structural). Is it trying to do too many things, or is this complexity inherent to its domain? Is this file truly dead, or is it used via a non-import mechanism (dynamic import, CLI entry point, plugin)?
    evidence: Flagged by: orphaned, smells, structural
    evidence: File size: 565 lines
    fingerprint: 1b0b0cd23a98614f
  ...
  - [mixed_responsibilities] rustfuzz/search.py
    summary: Issues from 3 detectors — may have too many responsibilities
    question: This file has issues across 3 dimensions (dupes, smells, structural). Is it trying to do too many things, or is this complexity inherent to its domain? Should the parameters be grouped into a config/context object? Which ones belong together? Can the nesting be reduced with early returns, guard clauses, or extraction into helper functions? Is the duplication worth extracting into a shared utility, or is it intentional variation?
    evidence: Flagged by: dupes, smells, structural
    evidence: File size: 1879 lines
...
1. Keep `session.id` exactly `ext_20260312_194516_998649ac`.
2. Keep `session.token` exactly `adde23ba63ff24e2e6021efef7900d33`.
